  ...
  <div ref="gridRef" class="w-full h-full overflow-hidden">
    <section class="relative flex justify-center items-center w-full h-screen overflow-hidden" :style="{
      ...
      <div class="z-[2] relative flex-none gap-4 grid grid-cols-1 w-[150vw] h-[150vh] rotate-[-15deg] origin-center">
        <div v-for="rowIndex in 10" :key="rowIndex" class="gap-4 flex" :style="{ willChange: 'transform, filter' }"
          ref="rowRefs">
          <div v-for="itemIndex in 15" :key="itemIndex" class="relative h-[250px] min-w-[300px] flex-shrink-0"
            ...
            <div
              ...
              <div v-if="isImage(combinedItems[(rowIndex - 1) * 15 + (itemIndex - 1)])"
              ...
              <div v-else-if="isTag(combinedItems[(rowIndex - 1) * 15 + (itemIndex - 1)])" class="z-[2] p-4 text-center"
                v-html="combinedItems[(rowIndex - 1) * 15 + (itemIndex - 1)]"></div>
